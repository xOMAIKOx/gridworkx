...
func _initialize() -> void:
	...
	controller.inspect_component("component.output_conveyor")
	controller.inspect_component("component.feed_conveyor")
	await _frames(2)
	await _capture_size(Vector2i(390, 844), "wp011-inspected-feed.png")
	controller.select_diagnosis("fault.motor_bearing_seizure")
	controller.repair_output_belt()
	controller.repair_feed_conveyor()
	await _frames(2)
	await _capture_size(Vector2i(390, 844), "wp011-partial-recovery.png")
	controller.produce_first_output()
	await _frames(2)
	await _capture_size(Vector2i(390, 844), "wp011-first-output.png")
	quit()

...
Every externally meaningful payload is associated with schema, rules/content and release versions. Semantic IDs use namespaces and never contain translated prose. WP-002 owns simulation command/snapshot schemas, WP-003 adds the strict facility graph schema, and WP-004 adds strict failure-state/failure-command contracts; Rust representations and these schemas must evolve together.

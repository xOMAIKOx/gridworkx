...
if 'godot = "=0.2.4"' not in manifest and 'version = "=0.2.4"' not in manifest: raise SystemExit("WP-010 godot binding is not exactly pinned to 0.2.4")
if 'gridworks-sim' not in manifest or 'crate-type = ["cdylib"]' not in manifest: raise SystemExit("WP-010 bridge crate contract is incomplete")
...
if "res://native/gridworks_sim.gdextension" not in (root / "ops/scripts/build-godot-extension.sh").read_text(): raise SystemExit("WP-010 build helper does not generate the extension list entry")
